...
`packages/VENDOR-SOURCE.json` 记录引擎仓唯一真源坐标。插件内副本只作为可安装快照；后续必须由受审同步脚本或版本化产物更新，禁止手工覆盖后不留坐标。当前本机门禁校验嵌套插件的 frozen install、类型、协议测试和提交 bundle 新鲜度；仓内 CI 仅允许全局 API 手动触发，不存在自动运行承诺。跨仓逐文件漂移门禁仍需在合并前补成版本化同步脚本。
