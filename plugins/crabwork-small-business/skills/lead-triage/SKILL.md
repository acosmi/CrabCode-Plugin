...
version: 0.3.0
description: >
  按互动信号、公司匹配度与紧迫度给 HubSpot 入站线索打分,产出"今天先打这 5 个"的
  ...
  HubSpot leads by engagement signals, company fit, and urgency markers to produce
  ...
  Feishu (飞书日历) calendar. Use when the user asks to prioritize leads, "who should
...
从 HubSpot 拉取入站线索,打分,产出一份带话术要点的排序通话清单。可替业主起草跟进、提议日历时段——未经业主批准绝不代发、绝不代订。
...
```
用户:"给我的线索排个序"
→ 拉取联系人:生命周期阶段 = Lead 或 MQL,状态 ≠ Unqualified
...
1. **从 HubSpot 拉取线索。** 拉取 `lifecyclestage` = `Lead` 或 `MQL` 且 `hs_lead_status` ≠ `Unqualified` 的联系人。字段清单见 [reference/hubspot-scoring.md](reference/hubspot-scoring.md)。若 HubSpot 未连接,停止:*"HubSpot 未连接——请先连接后再试。"*(CRM 连接器以实际配置为准。)

2. **触发词含糊先澄清。** 若业主只说"管道"而无限定词,先问:*"要快速的管道概览(各成交阶段 + 总金额),还是排好序的通话清单?"*——再据此路由。仅凭一个"管道"不要直接给线索打分。

3. **逐条打分。** 套用 [reference/hubspot-scoring.md](reference/hubspot-scoring.md) 的四维模型:
   - **互动** —— HubSpot 中的邮件回复、打开、网站访问(仅近 30 天)
...
5. **提议起草跟进。** 问:*"要给其中哪些起草跟进?"* 若要,每个选定线索写一封邮件,语气与背景取自该线索的 HubSpot 备注与已记录活动——没有邮件连接器,若业主要精确衔接上一封往来,请其粘贴原文。草稿在对话中展示;由业主自行复制到自己的邮箱工具或微信发送。绝不代发。
...
2. **最小必要。** 只使用实现"排序跟进"目的所必需的字段(见 hubspot-scoring.md 的字段清单),不为打分而过度收集或引入无关个人信息。
...
- **绝不把 `Customer` 或 `Evangelist` 生命周期的联系人**纳入线索清单。
- **打分与跟进基于合法取得的个人信息,仅作内部优先级排序,不作价格歧视等差别待遇。**
- **若无线索匹配过滤条件**,解释原因并提议查看当前在用的生命周期阶段——不要编造清单。

## 参考

- [reference/hubspot-scoring.md](reference/hubspot-scoring.md) —— HubSpot 字段名、打分权重、ICP 默认值(CRM 连接器以实际配置为准)
