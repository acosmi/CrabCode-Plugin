...
description: >
  ...
  into profit, or how a price change might affect their business. Trigger even
...
> **Status:** MVP draft · **Owner:** JJ · **Version:** 0.3.0
...
**Industry:** Benchmarking needs the owner's business category. If it isn't already known from the conversation, ask: "I need your business category to pull relevant benchmarks. What industry are you in?"

**Data files:** This skill runs entirely from exports and pasted data — there is no live accounting or payment-history connector. (The Alipay MCP connector only creates payment links, queries single payments, and handles refunds; it cannot export transaction history.) The expected CSV shapes are in `reference/csv-schema.md`.
...
1. **"Which products or services do you want to analyze?"**
   ...
   - If they say "all," confirm the exports have enough data to be meaningful before analyzing everything.
...
### Step 3: Collect cost data (accounting software)

Ask the owner for an export from their accounting software (用友好会计 / 金蝶精斗云) — a P&L detail or expense report as CSV/Excel, or a pasted copy:
...
If no export is available, ask the owner for:
...
If the export shows COGS = ¥0 across all periods, do not use ¥0 as the cost input. Surface this to the owner:

> "Your accounting export shows no cost of goods sold recorded for this period. To compute meaningful margins, I need a cost breakdown by product or service line — not a single average for the whole business. For each item you want analyzed, what does it cost you to deliver it? Materials, direct labor, any direct expenses per item. Even rough figures work."

Flag this limitation in the Data Quality Notes section of the final output.

### Step 4: Collect revenue data (Alipay / WeChat Pay bill exports)

Ask the owner for a 支付宝商家平台 bill export (CSV) — and a 微信支付商户平台 bill export if they also collect there (WeChat Pay is not yet connected; the owner downloads it manually):
...
If the bill export doesn't carry item names (payment bills often show only order numbers), ask the owner for an order export from their shop/order system, or a mapping from order to product.
...
| **COGS** | Cost data from the accounting export or owner-provided |
...
**Primary:** Accounting software export (用友好会计 / 金蝶精斗云) for costs; 支付宝商家平台 bill export for revenue
**Also supported:** 微信支付商户平台 bill export (WeChat Pay — not yet connected; manual download) · Pasted data / any CSV export

The Alipay MCP connector is for payment links and refunds only — revenue history always comes from bill exports.
