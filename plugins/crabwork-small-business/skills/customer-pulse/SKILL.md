...
description: >
  ...
  你的 CRM 反馈与客服工单、在线客服会话、粘贴进来的客户邮件,以及粘贴或导出的
...
CrabCode 拉取近 30 天的客服工单与在线客服会话(若已连接),叠加店主提供的一切材料——支付纠纷/退款导出、粘贴的客户邮件、粘贴的评价,把所有材料归成 3–5 个主题(每个主题配逐字证据),再交付一份"本周就做这三件事"的行动清单。
...
3. **从你的 CRM 拉取客服工单与反馈(CRM 连接器以实际配置为准)。** 拉取未结及近期已结工单。若工单数为 0,记 `客服工单:0` 并继续——不要弹警告。
...
5. **拉取在线客服会话(若已连接)。** 从你的客服工单 / 在线客服系统(若已连接)拉取未结及近期已结的客服会话,只取客户发言的部分——排除客服与机器人的发言。若未连接,请店主把会话文本粘贴进来即可。

6. **接受粘贴的评价(可选)。** 若店主粘贴大众点评或淘宝评价文本(或任何其他评价平台的导出),纳入来源池并标记为 `[评价]`。无需连接器。

7. **提炼主题。** 把全部证据归成 3–5 个反复出现的主题。每个主题须含:
   - 一句话标签(例如"配送延误引发反复投诉")
   - 2–3 条**逐字引用**,并标注来源:`[支付纠纷]`、`[CRM]`、`[邮件]`、`[客服]` 或 `[评价]`
   - 信号计数(有多少条材料触及该主题)
...
9. **交付报告。** 按以下顺序组织输出:
   ...
   - **来源盘点** —— 列出各来源的信号计数(支付纠纷/退款、客服工单、粘贴邮件、在线客服会话、粘贴评价)。未提供而跳过的来源要注明。
...
- [reference/gotchas.md](reference/gotchas.md) —— 缺支付导出、客服工单空结果、逐字引用要求与去标识化、邮件关键词漂移
