...
If CrabCode embeds them as markdown images (`![](design.canva.ai/...)`), they
...
Either let the native CrabCode carousel render the tool result automatically,
...
**Why it matters:** When CrabCode lists each design as a separate block
("Hero 1 / [image] / Hero 2 / [image]"), the images render as broken
gray "Show Image" placeholders. Presenting all designs together in a
single grouped batch triggers CrabCode's side-by-side carousel, which
renders correctly.
...
CrabCode guesses a template by name and generates with the wrong one, the owner
