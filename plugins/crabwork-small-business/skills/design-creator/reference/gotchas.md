...
## 坑:把国内平台行走 HubSpot Social API(那是外贸 / 海外社媒附录)

**为什么要紧:** HubSpot Social(外贸 / 海外社媒附录)只能自动发布到在 HubSpot 内已连接的海外账号(Instagram / Facebook 等);公众号、
小红书、抖音、视频号无法接入。对这些国内行调用外贸 Social API,要么报错,更糟的是把帖暂存
到店主恰好连着的另一个(海外)错误渠道。
...
按渠道拆分阶段 5:做外贸、有 HubSpot 可连接的海外账号(Instagram / Facebook)的行才暂存为 `SCHEDULED`;公众号/小红书/抖音/
视频号 行拿到可直接发布的打包件(日期/时间 + 最终文案 + 已核验素材)外加排期 CSV,
登记到你的 CRM / 营销系统战役下以便追踪。店主原生发布——公众号在自己的编辑器里支持定时发布。
...
**为什么要紧:** 过去的发布时间发不出去——外贸社媒 API(如 HubSpot)会以校验错误拒绝 `scheduledAt` 在过去的帖,排期 CSV 里排到过去的时间同样作废。若日历是在更早
的日期搭的、店主现在才来排期 / 暂存,有些日期可能已经过去。

### ✗ 坏

4月1日 为 6 月的帖搭日历 → 店主 5月15日 批准 → 不检查就排期 → "4月30日 10:00" 的帖
已排到过去(外贸 HubSpot 行返回 400,国内行则成了排期 CSV 里的废时间)。

### ✓ 好

生成排期 CSV / 调用外贸 `POST /social/posts` 前,把每个发布时间与当前时间比对。若有日期已
