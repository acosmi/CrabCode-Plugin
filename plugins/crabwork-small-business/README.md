...
**历史国内直连候选**（本版本未内置可执行连接；不要填写凭证，工作流使用手工导入降级路径）：
...
> **CRM 数据口径**：当前不连接或推荐 CRM。技能只处理用户导出的客户、线索、订单或工单数据，并以中性能力概念描述工作流；历史候选仅见 [CONNECTORS.md](CONNECTORS.md)。

> **收款数据口径**：当前不调用任何支付 MCP。现金流、对账所需数据由用户从商户平台导出账单(CSV)后交给技能分析。

当前无需且不得连接外部工具；先用导出文件、粘贴数据和人工执行完成工作流。
