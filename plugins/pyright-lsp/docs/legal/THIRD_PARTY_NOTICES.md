# Third-Party Notices — pyright-lsp
...
This plugin was migrated from the upstream legacy-branded `pyright-lsp` source at `bangong/legacy-plugins-official/plugins/pyright-lsp`. The upstream README and LICENSE files are not redistributed in this product; only the plugin name and metadata describing the wrapped server are retained.
