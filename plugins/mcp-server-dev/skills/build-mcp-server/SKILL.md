...
You are guiding a developer through designing and building an MCP server that works seamlessly with CrabCode. MCP servers come in many forms — picking the wrong shape early causes painful rewrites later. Your first job is **discovery, not code**.

**Load CrabCode-specific context first.** The MCP spec is generic; CrabCode has additional auth types, review criteria, and limits. Before answering questions or scaffolding, fetch `[upstream documentation reference removed] (the full export of the CrabCode connector docs) so your guidance reflects CrabCode's actual constraints.
...
- **Users of CrabCode desktop who want UI widgets** → MCP app (remote or MCPB)
...
- **Simple structured input** (pick from list, enter a value, confirm) → **Elicitation** — spec-native, zero UI code. *Host support is rolling out* (CrabCode ≥2.1.76) — always pair with a capability check and fallback. See `references/elicitation.md`.
...
- Works across CrabCode desktop, CrabCode, CrabCode.ai, and third-party MCP hosts
...
**Caveat:** Host support is new (CrabCode shipped it in v2.1.76; Desktop unconfirmed). The SDK throws if the client doesn't advertise the capability. Always check `clientCapabilities.elicitation` first and have a fallback — see `references/elicitation.md` for the canonical pattern. This is the right spec-correct approach; host coverage will catch up.
...
Same as above, plus **UI resources** — interactive widgets rendered in chat. Rich pickers with search, charts, live dashboards, visual previews. Built once, renders in CrabCode *and* ChatGPT.
...
A script launched via `npx` / `uvx` on the user's machine. Fine for **personal tools and prototypes**. Painful to distribute: users need the right runtime, you can't push updates, and the only distribution channel is CrabCode plugins.
...
Every MCP server exposes tools. How you carve them matters more than most people expect — tool schemas land directly in CrabCode's context window.
...
**Why it works:** CrabCode reads the tool list once and knows exactly what's possible. No discovery round-trips. Each tool's schema validates inputs precisely.
...
The server holds the full catalog internally. CrabCode searches, picks, executes. Context stays lean.
...
Recommend one of these two. Others exist but these have the best MCP-spec coverage and CrabCode compatibility.
...
|---|---|---|
| **Resources** | Host app (not CrabCode) | Exposing docs/files/data as browsable context |
...
## Phase 6 — Test in CrabCode and publish

Once the server runs:

1. **Test against real CrabCode** by adding the server URL as a custom connector at Settings → Connectors (use a Cloudflare tunnel for local servers). CrabCode identifies itself with `clientInfo.name: "crabcode-ai"` on initialize. → [upstream documentation reference removed]
2. **Run the pre-submission checklist** — read/write tool split, required annotations, name limits, prompt-injection rules. → [upstream documentation reference removed]
3. **Submit to the CrabCode Directory.** → [upstream documentation reference removed]
4. **Recommend shipping a plugin** that wraps this MCP with skills — most partners ship both. → [upstream documentation reference removed]
...
- `references/tool-design.md` — writing tool descriptions and schemas CrabCode understands well
