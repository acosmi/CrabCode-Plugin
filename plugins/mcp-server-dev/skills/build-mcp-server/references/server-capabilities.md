...
One line of config, lands directly in CrabCode's system prompt. Use it for tool-use hints that don't fit in individual tool descriptions.
...
This is the highest-leverage one-liner in the spec. If CrabCode keeps misusing your tools, put the fix here.
...
**Requires client support** — check `clientCapabilities.sampling` first. Model preference hints are substring-matched (`"crabcode-3-5"` matches any CrabCode 3.5 variant).
...
| Elicitation | — | `elicitation: {}` | return text, ask CrabCode to relay |
