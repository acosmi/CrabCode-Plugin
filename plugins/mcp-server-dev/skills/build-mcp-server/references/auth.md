...
## CrabCode-specific authentication

CrabCode's MCP client supports a specific set of auth types — not every spec-compliant flow works. Full reference: [upstream documentation reference removed]

| Type | Notes |
|---|---|
| `oauth_dcr` | Supported. For high-volume directory entries, prefer CIMD or CrabCode-held creds — DCR registers a new client on every fresh connection. |
| `oauth_cimd` | Supported, recommended over DCR for directory entries. |
| `oauth_provider_creds` | Partner provides `client_id`/`client_secret` to CrabCode; user-consent-gated. Contact `support@crabcode.dev`. |
| `custom_connection` | User supplies URL/creds at connect time (Snowflake-style). Contact `support@crabcode.dev`. |
| `none` | Authless. |
...
**Callback URL** (single, all surfaces): `[upstream reference removed]
