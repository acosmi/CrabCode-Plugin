...
FastMCP derives the JSON schema from type hints and the docstring becomes the tool description. Keep docstrings terse and action-oriented — they land in CrabCode's context window verbatim.
...
| Surface | How |
|---|---|
| **CrabCode** | `crabcode mcp add --transport http <name> <url>` (add `--scope user` for global, `--header "Authorization: Bearer ..."` for auth) |
| **CrabCode Desktop / CrabCode.ai** | Settings → Connectors → Add custom connector. **Not** `host_mcp_config.json` — remote servers configured there are ignored. |
| **Connector directory** | CrabCode maintains a submission guide for listing in the public connector directory. |
