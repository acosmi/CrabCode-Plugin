...
Validated against `github.com/crabcode-team/mcpb/schemas/mcpb-manifest-v0.4.schema.json`. The schema uses `additionalProperties: false` — unknown keys are rejected. Add `"$schema"` to your manifest for editor validation.
...
| Field | Description |
|---|---|
| `crabcode_desktop` | Semver range. Install blocked if host is older. |
...
```json
{
  "$schema": "[upstream reference removed]
...
- **No `minHostVersion`.** It's `compatibility.crabcode_desktop`.
