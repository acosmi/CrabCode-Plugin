...
ID. Traffic from crabcode.ai is proxied through CrabCode's egress — every web
...
(See https://platform.crabcode.com/docs/en/api/ip-addresses.)

CrabCode Desktop, CrabCode, and other hosts connect **directly from the
user's machine**, so those *do* have distinct per-user IPs. Per-IP limiting
therefore works for direct-connect clients; for crabcode.ai you can only limit
the aggregate CrabCode pool. If true per-user limits matter, that's the
trigger to add OAuth.

## Tiered token-bucket (per-replica backstop)

```ts
const ALLOWED_CIDRS = ["160.79.104.0/21", "2607:6bc0::/48"];
const TIERS = {
  crabcode: { capacity: 600, refillPerSec: 100 }, // shared pool
  other:     { capacity: 30,  refillPerSec: 2   }, // per-IP
};
```

Match `req.ip` against the CIDRs, pick a bucket (`"crabcode"` or
...
`X-Forwarded-For: 160.79.108.42` and claim the CrabCode tier. Set it to the
exact number of trusted hops (e.g. `1` behind a single LB, `2` behind
Cloudflare → origin LB) and **never `true` in production**.

## Hard-allowlisting CrabCode IPs is a product decision

Blocking everything outside `160.79.104.0/21` locks out Desktop, CrabCode,
and every other MCP host. Use the CIDRs to **tier** rate limits, not to gate
access, unless crabcode.ai-only is an explicit goal.
