...
First, CrabCode should gather information about the user's project from Skill 1:
...
CrabCode should work with the user to list EVERY assumption the project makes from inception through conclusion. Assumptions fall into two categories:
...
For each assumption, CrabCode should help the user assign two scores:
...
Once the complete table is ready, CrabCode should analyze the risk profile:
...
For each high-risk assumption (level 4-5), CrabCode should help develop mitigation strategies:
...
For the top 3 highest-risk assumptions, CrabCode should help design the critical go/no-go experiments:
...
CrabCode should search PubMed to help calibrate risk assessments:
...
Based on the risk analysis, CrabCode should help create a revised plan:
...
CrabCode should produce a **2-page Risk Assessment Document**:
...
CrabCode should begin with Phase 1 by asking for:
1. The project summary from Skill 1
2. Project timeline expectations
3. What concerns the user most about this project

Together, CrabCode and the user will build a rigorous risk assessment that dramatically improves the likelihood of success by helping avoid years of work on projects with insurmountable obstacles.
