...
This plugin was migrated from the upstream legacy-branded `rust-analyzer-lsp` source at `bangong/legacy-plugins-official/plugins/rust-analyzer-lsp`. The upstream README and LICENSE files are not redistributed in this product; only the plugin name and metadata describing the wrapped server are retained.
