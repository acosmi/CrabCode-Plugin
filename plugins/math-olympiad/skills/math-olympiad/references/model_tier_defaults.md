...
the asymmetric noise floor (<model-id> verifiers are individually less reliable, so
...
- **Pattern sweep**: all 12 patterns — <model-id> can follow a checklist, the
  patterns are the scaffold
- **Presentation pass**: yes, 3 drafts, comparator picks cleanest. <model-id>'s raw
  output is rougher, so this matters MORE not less.
- **Rationale**: The skill's value is highest where the base model is weakest.
  Give <model-id> the full harness. The 3-refute threshold (higher than <model-id>'s 2)
  accounts for <model-id> verifiers being individually noisier — don't let 2 confused
  <model-id> kill a correct proof.
...
- **Rationale**: <model-id> can execute the deep patterns (#19 base-vs-derived, #22
...
If the orchestrating session doesn't know which model it is, default to <model-id>
configuration. A reasonable heuristic: ask the model to self-identify in its
first response and match against `<model-id>`/`<model-id>`/`<model-id>` in the output.
