...
/**
 * iMessage channel for CrabCode — direct chat.db + AppleScript.
 *
 ...
 * support. State in ~/.crabcode/channels/imessage/access.json, managed by the
...
const STATE_DIR = process.env.IMESSAGE_STATE_DIR ?? join(homedir(), '.crabcode', 'channels', 'imessage')
...
// The agent can already Read+paste file contents, so this isn't a new exfil
// channel for arbitrary paths — but the server's own state is the one thing
// the agent has no reason to ever send. No inbox carve-out: iMessage attachments
...
function checkApprovals(): void {
  ...
  for (const senderId of files) {
    ...
    const err = sendText(chatGuid, "Paired! Say hi to the agent.")
