...
| Config file | `~/.crabcode/channels/telegram/access.json` |
...
```
/telegram:access set mentionPatterns '["^hey crabby\\b", "\\bassistant\\b"]'
```
...
`~/.crabcode/channels/telegram/access.json`. Absent file is equivalent to `pairing` policy with empty lists, so the first DM triggers pairing.
