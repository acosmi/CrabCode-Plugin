...
This plugin was migrated from the upstream Claude-branded `swift-lsp` source at `bangong/claude-plugins-official/plugins/swift-lsp`. The upstream README and LICENSE files are not redistributed in this product; only the plugin name and metadata describing the wrapped server are retained.
