...
This plugin was migrated from the upstream legacy-branded `gopls-lsp` source at `bangong/legacy-plugins-official/plugins/gopls-lsp`. The upstream README and LICENSE files are not redistributed in this product; only the plugin name and metadata describing the wrapped server are retained.
