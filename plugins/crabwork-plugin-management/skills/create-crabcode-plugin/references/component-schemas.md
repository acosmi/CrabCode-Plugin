...
| `Stop`             | When CrabCode finishes a response |
  ...
  "SessionStart": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "cat ${CRABCODE_PLUGIN_ROOT}/context/project-context.md",
          "timeout": 10
        }
      ]
    }
...
> **Prefer `skills/*/SKILL.md` for new plugins.** CrabCode now presents commands and skills as a single "Skills" concept. The `commands/` format still works, but only use it if you specifically need the single-file format with `$ARGUMENTS`/`$1` substitution and inline bash execution.
...
- Commands are instructions FOR CrabCode, not messages for the user. Write them as directives.
...
- Use `${CRABCODE_PLUGIN_ROOT}` to reference plugin files portably.
