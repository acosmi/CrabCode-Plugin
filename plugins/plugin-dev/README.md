...
A comprehensive toolkit for developing CrabCode plugins with expert guidance on hooks, MCP integration, plugin structure, and marketplace publishing.

## Overview

The plugin-dev toolkit provides seven specialized skills to help you build high-quality CrabCode plugins:
...
4. **plugin-settings** - Configuration patterns using .crabcode/plugin-name.local.md files
...
7. **Testing** - Verify plugin works in CrabCode
...
**Trigger phrases:** "create a hook", "add a PreToolUse hook", "validate tool use", "implement prompt-based hooks", "${CRABCODE_PLUGIN_ROOT}", "block dangerous commands"
...
- ${CRABCODE_PLUGIN_ROOT} usage throughout
...
**What it covers:**
- .crabcode/plugin-name.local.md pattern for configuration
...
**Trigger phrases:** "create an agent", "add an agent", "write a agent", "agent frontmatter", "when to use description", "agent examples", "autonomous agent"
...
- AI-assisted agent generation using CrabCode's proven prompt
...
This keeps CrabCode's context focused while providing deep knowledge when needed.
...
- Based on official CrabCode documentation
...
✅ **Portability**
- Use ${CRABCODE_PLUGIN_ROOT} everywhere
- Relative paths only
