...
Guide the user through creating a complete, high-quality CrabCode plugin from initial concept to tested implementation. Follow a systematic approach: understand requirements, design components, clarify details, implement following best practices, validate, and test.
...
3. Create directory structure using bash:
   ```bash
   mkdir -p plugin-name/.crabcode-plugin
...
6. Create .gitignore if needed (for .crabcode/\*.local.md, etc.)
...
2. For each skill:
   ...
   - For user-invoked skills (slash commands): include `description`, `argument-hint`, and `allowed-tools` frontmatter; write instructions FOR CrabCode (not TO user)
...
2. For each command:
   ...
   - Write instructions FOR CrabCode (not TO user)
...
3. Create example .crabcode/plugin-name.local.md file (as documentation)
4. Implement settings reading in hooks/commands as needed
5. Add to .gitignore: `.crabcode/*.local.md`
...
5. **Test hook configuration** (if plugin has hooks):
   ...
   - Verify ${CRABCODE_PLUGIN_ROOT} usage
...
**Goal**: Test that plugin works correctly in CrabCode
...
- **Apply best practices**:
  ...
  - Skill instructions written FOR CrabCode (not TO user)
