...
When CrabCode starts:

1. **Scan enabled plugins**: Read `.crabcode-plugin/plugin.json` for each
2. **Discover components**: Look in default and custom paths
3. **Parse definitions**: Read YAML frontmatter and configurations
4. **Register components**: Make available to CrabCode
5. **Initialize**: Start MCP servers, register hooks

**Timing**: Component registration happens during CrabCode initialization, not continuously.

### Activation Phase

When components are used:

**Commands**: User types slash command → CrabCode looks up → Executes
**Agents**: Task arrives → CrabCode evaluates capabilities → Selects agent
**Skills**: Task context matches description → CrabCode loads skill
**Hooks**: Event occurs → CrabCode calls matching hooks
...
**Note**: CrabCode doesn't support nested command discovery automatically. Use custom paths:
...
**Note**: Use build script to combine files, CrabCode doesn't support file references.
...
```bash
#!/bin/bash
source "${CRABCODE_PLUGIN_ROOT}/lib/test-utils.sh"
run_tests
```
