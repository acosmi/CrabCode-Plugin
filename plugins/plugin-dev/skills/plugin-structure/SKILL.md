...
- Manifest-driven configuration in `.crabcode-plugin/plugin.json`
- Component-based organization (commands, agents, skills, hooks)
- Portable path references using `${CRABCODE_PLUGIN_ROOT}`
- Explicit vs. auto-discovered component loading

## Directory Structure

Every CrabCode plugin follows this organizational pattern:
...
1. **Manifest location**: The `plugin.json` manifest MUST be in `.crabcode-plugin/` directory
2. **Component locations**: All component directories (commands, agents, skills, hooks) MUST be at plugin root level, NOT nested inside `.crabcode-plugin/`
3. **Optional components**: Only create directories for components the plugin actually uses
4. **Naming convention**: Use kebab-case for all directory and file names

## Plugin Manifest (plugin.json)

The manifest defines plugin metadata and configuration. Located at `.crabcode-plugin/plugin.json`:
...
**Usage**: Commands integrate as native slash commands in CrabCode
...
**Usage**: Users can invoke agents manually, or CrabCode selects them automatically based on task context
...
**Usage**: CrabCode autonomously activates skills based on task context matching the description
...
```json
{
  "PreToolUse": [{
    ...
    "hooks": [{
      "type": "command",
      "command": "bash ${CRABCODE_PLUGIN_ROOT}/hooks/scripts/validate.sh",
      "timeout": 30
    }]
  }]
}
...
Use `${CRABCODE_PLUGIN_ROOT}` environment variable for all intra-plugin path references:

```json
{
  "command": "bash ${CRABCODE_PLUGIN_ROOT}/scripts/run.sh"
}
```
...
```json
"command": "${CRABCODE_PLUGIN_ROOT}/scripts/tool.sh"
```

**In component files** (commands, agents, skills):
```markdown
Reference scripts at: ${CRABCODE_PLUGIN_ROOT}/scripts/helper.py
```

**In executed scripts**:
```bash
#!/bin/bash
# ${CRABCODE_PLUGIN_ROOT} available as environment variable
source "${CRABCODE_PLUGIN_ROOT}/lib/common.sh"
```
...
CrabCode automatically discovers and loads components:

1. **Plugin manifest**: Reads `.crabcode-plugin/plugin.json` when plugin enables
...
1. **Always use ${CRABCODE_PLUGIN_ROOT}**: Never hardcode paths
...
- Confirm plugin is enabled in CrabCode settings

**Path resolution errors**:
- Replace all hardcoded paths with `${CRABCODE_PLUGIN_ROOT}`
- Verify paths are relative and start with `./` in manifest
- Check that referenced files exist at specified paths
- Test with `echo $CRABCODE_PLUGIN_ROOT` in hook scripts

**Auto-discovery not working**:
- Confirm directories are at plugin root (not in `.crabcode-plugin/`)
- Check file naming follows conventions (kebab-case, correct extensions)
- Verify custom paths in manifest are correct
- Restart CrabCode to reload plugin configuration
