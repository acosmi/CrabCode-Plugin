...
description: Use this agent when the user asks to deploy across multiple environments, coordinate a staged rollout, or roll back a deployment. Typical triggers include multi-stage releases, dependency-ordered service deploys, and post-deploy health checks.
model: inherit
---
...
\`\`\`javascript
const { DatadogClient } = require('${CRABCODE_PLUGIN_ROOT}/lib/integrations/datadog')
...
\`\`\`javascript
const { SlackClient } = require('${CRABCODE_PLUGIN_ROOT}/lib/integrations/slack')
...
\`\`\`bash
bash ${CRABCODE_PLUGIN_ROOT}/skills/kubernetes-ops/scripts/validate-manifest.sh deployment.yaml
\`\`\`
```
  ...
  "PreToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ${CRABCODE_PLUGIN_ROOT}/hooks/scripts/security/scan-secrets.sh",
          "timeout": 30
        }
      ]
    },
  ...
  "PostToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ${CRABCODE_PLUGIN_ROOT}/hooks/scripts/workflow/update-status.sh",
          "timeout": 15
        }
      ]
    }
  ...
  "Stop": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ${CRABCODE_PLUGIN_ROOT}/hooks/scripts/quality/check-config.sh",
          "timeout": 45
        },
        {
          "type": "command",
          "command": "bash ${CRABCODE_PLUGIN_ROOT}/hooks/scripts/workflow/notify-team.sh",
          "timeout": 30
        }
      ]
    }
  ...
  "SessionStart": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ${CRABCODE_PLUGIN_ROOT}/hooks/scripts/security/validate-permissions.sh",
          "timeout": 20
        }
      ]
    }
