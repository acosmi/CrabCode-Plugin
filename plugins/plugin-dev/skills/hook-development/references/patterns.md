...
This reference provides common, proven patterns for implementing CrabCode hooks. Use these patterns as starting points for typical hook use cases.
...
if [ -f "package.json" ]; then
  ...
  echo "export PROJECT_TYPE=rust" >> "$CRABCODE_ENV_FILE"
fi
```
  ...
  "Notification": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ${CRABCODE_PLUGIN_ROOT}/scripts/log-notification.sh"
        }
      ]
    }
  ]
  ...
  "PostToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ${CRABCODE_PLUGIN_ROOT}/scripts/check-quality.sh"
        }
      ]
    }
  ]
...
FLAG_FILE="$CRABCODE_PROJECT_DIR/.enable-security-scan"
...
```bash
#!/bin/bash
CONFIG_FILE="$CRABCODE_PROJECT_DIR/.crabcode/my-plugin.local.json"
...
**Configuration file (.crabcode/my-plugin.local.json):**
```json
{
  "strictMode": true,
  "maxFileSize": 500000,
