...
VALID_EVENTS=("PreToolUse" "PostToolUse" "UserPromptSubmit" "Stop" "AgentStop" "SessionStart" "SessionEnd" "PreCompact" "Notification")
    ...
    for ((j=0; j<hook_array_count; j++)); do
      ...
      if [ "$hook_type" = "command" ]; then
        ...
        if [ -z "$command" ]; then
          ...
          if [[ "$command" == /* ]] && [[ "$command" != *'${CRABCODE_PLUGIN_ROOT}'* ]]; then
        ...
        if [ "$event" != "Stop" ] && [ "$event" != "AgentStop" ] && [ "$event" != "UserPromptSubmit" ] && [ "$event" != "PreToolUse" ]; then
          echo "⚠️  $event[$i].hooks[$j]: Prompt hooks may not be fully supported on $event (best on Stop, AgentStop, UserPromptSubmit, PreToolUse)"
          ((warning_count++))
        fi
      fi
