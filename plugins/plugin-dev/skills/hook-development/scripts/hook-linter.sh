...
check_script() {
  ...
  if grep -E '^[^#]*/home/|^[^#]*/usr/|^[^#]*/opt/' "$script" | grep -q .; then
    ...
    echo "   Use \$CRABCODE_PROJECT_DIR or \$CRABCODE_PLUGIN_ROOT"
    warnings=$((warnings + 1))
  fi

  # Check 8: Uses CRABCODE_PLUGIN_ROOT
  if ! grep -q "CRABCODE_PLUGIN_ROOT\|CRABCODE_PROJECT_DIR" "$script"; then
    echo "💡 Tip: Use \$CRABCODE_PLUGIN_ROOT for plugin-relative paths"
  fi
  ...
  if grep -E 'sleep [0-9]{3,}|while true' "$script" | grep -v '#' | grep -q .; then
    ...
    echo "   Hooks are killed at their timeout (600s by default for tool hooks,"
    echo "   1.5s for SessionEnd) — keep them well under it"
    warnings=$((warnings + 1))
  fi
