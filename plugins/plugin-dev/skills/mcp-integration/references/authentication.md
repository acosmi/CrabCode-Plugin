...
Complete guide to authentication methods for MCP servers in CrabCode plugins.
...
CrabCode automatically handles the complete OAuth 2.0 flow for SSE and HTTP servers:

1. User attempts to use MCP tool
2. CrabCode detects authentication needed
3. Opens browser for OAuth consent
4. User authorizes in browser
5. Tokens stored securely by CrabCode
...
No additional auth configuration needed! CrabCode handles everything.
...
```json
{
  "api": {
    ...
    "headersHelper": "${CRABCODE_PLUGIN_ROOT}/scripts/get-headers.sh"
  }
}
```
...
{
  "secure-api": {
    "command": "${CRABCODE_PLUGIN_ROOT}/servers/mtls-wrapper",
...
```json
{
  "headersHelper": "${CRABCODE_PLUGIN_ROOT}/scripts/generate-jwt.sh"
}
```
