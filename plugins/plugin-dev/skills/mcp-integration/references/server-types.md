...
Complete reference for all MCP server types supported in CrabCode plugins.
...
{
  "my-server": {
    "command": "${CRABCODE_PLUGIN_ROOT}/servers/custom-server",
...
1. **Startup**: CrabCode spawns process with `command` and `args`
2. **Communication**: JSON-RPC messages via stdin/stdout
3. **Lifecycle**: Process runs for entire CrabCode session
4. **Shutdown**: Process terminated when CrabCode exits
...
{
  "custom": {
    "command": "${CRABCODE_PLUGIN_ROOT}/servers/my-server.js",
    "args": ["--verbose"]
  }
}
```
...
1. **Use absolute paths or ${CRABCODE_PLUGIN_ROOT}**
