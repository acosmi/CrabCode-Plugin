...
Comprehensive guidance on creating CrabCode slash commands, including file format, frontmatter options, dynamic arguments, and best practices.
...
- Plugin-specific features (${CRABCODE_PLUGIN_ROOT}, plugin patterns)
...
- **plugin-features-reference.md**: Plugin-specific command features
  - Plugin command discovery and organization
  - ${CRABCODE_PLUGIN_ROOT} environment variable usage
  - Plugin command patterns (configuration, template, multi-script)
...
CrabCode activates this skill when users:
...
CrabCode loads references and examples as needed based on task.
...
- **Project**: `.crabcode/commands/` (shared with team)
- **Personal**: `~/.crabcode/commands/` (your commands)
...
- ✓ Plugin command patterns (${CRABCODE_PLUGIN_ROOT}, discovery, organization)
...
6. Test examples work with current CrabCode
