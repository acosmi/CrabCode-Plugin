...
if [ ! -f ".crabcode/command-initialized" ]; then
...
if [ -f ".crabcode/plugin-name.local.md" ]; then
  # Parse YAML frontmatter for settings
  VERBOSE=$(grep "^verbose:" .crabcode/plugin-name.local.md | cut -d: -f2 | tr -d ' ')
  COLOR=$(grep "^color:" .crabcode/plugin-name.local.md | cut -d: -f2 | tr -d ' ')
  MAX_RESULTS=$(grep "^max_results:" .crabcode/plugin-name.local.md | cut -d: -f2 | tr -d ' ')

  echo "✓ Using user configuration"
else
  echo "Using default configuration"
  echo "Create .crabcode/plugin-name.local.md to customize"
fi
...
if [ -f ".crabcode/operation-completed.flag" ]; then
  ℹ️  Operation already completed

  Completed at: $(cat .crabcode/operation-completed.flag)

  To re-run:
  1. Remove flag: rm .crabcode/operation-completed.flag
  2. Run command again
...
Marking complete...
echo "$(date)" > .crabcode/operation-completed.flag
```
