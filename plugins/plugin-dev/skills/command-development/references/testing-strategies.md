...
head -n 20 .crabcode/commands/my-command.md | grep -A 10 "^---"

# Check for closing frontmatter marker
head -n 20 .crabcode/commands/my-command.md | grep -c "^---" # Should be 2

# Verify file has .md extension
ls .crabcode/commands/*.md

# Check file is in correct location
test -f .crabcode/commands/my-command.md && echo "Found" || echo "Missing"
```
...
cat > .crabcode/commands/test-bash.md << 'EOF'
---
...
cat > .crabcode/commands/test-forbidden.md << 'EOF'
---
...
# Command: .crabcode/commands/risky-operation.md
...
COMMANDS_CHANGED=$(git diff --cached --name-only | grep "\.crabcode/commands/.*\.md")
  ...
  test:
    ...
    steps:
      ...
      - name: Validate command structure
        run: |
          for cmd in .crabcode/commands/*.md; do
            echo "Testing: $cmd"
      ...
      - name: Check for TODOs
        run: |
          if grep -r "TODO" .crabcode/commands/; then
...
```bash
# Monitor CrabCode during command execution
# In terminal 1:
crabcode --debug

# In terminal 2:
watch -n 1 'ps aux | grep crabcode'
...
```bash
# Check file location
ls -la .crabcode/commands/my-command.md

# Check permissions
chmod 644 .crabcode/commands/my-command.md

# Check syntax
head -n 20 .crabcode/commands/my-command.md
...
```bash
# Check allowed-tools
grep "allowed-tools" .crabcode/commands/my-command.md

# Verify command syntax
grep '!\`' .crabcode/commands/my-command.md
