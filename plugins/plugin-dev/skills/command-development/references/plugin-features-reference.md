...
This reference covers features and patterns specific to commands bundled in CrabCode plugins.

## Table of Contents

- [Plugin Command Discovery](#plugin-command-discovery)
- [CRABCODE_PLUGIN_ROOT Environment Variable](#crabcode_plugin_root-environment-variable)
...
CrabCode automatically discovers commands in plugins using the following locations:
...
## CRABCODE_PLUGIN_ROOT Environment Variable

### Purpose

`${CRABCODE_PLUGIN_ROOT}` is a special environment variable available in plugin commands that resolves to the absolute path of the plugin directory.
...
Run analysis: !`node ${CRABCODE_PLUGIN_ROOT}/scripts/analyze.js`

Read template: @${CRABCODE_PLUGIN_ROOT}/templates/report.md
```
...
1. **Always use for plugin-internal paths:**
   ```markdown
   # Good
   @${CRABCODE_PLUGIN_ROOT}/templates/foo.md
...
2. **Validate file existence:**
   ...
   !`test -f ${CRABCODE_PLUGIN_ROOT}/config.json && echo "exists" || echo "missing"`

   If config exists, load it: @${CRABCODE_PLUGIN_ROOT}/config.json
...
- Verify syntax is exact: `${CRABCODE_PLUGIN_ROOT}`
...
Validate: !`bash ${CRABCODE_PLUGIN_ROOT}/scripts/validate.sh`
...
- CrabCode will automatically use Task tool to launch agent
...
- Mention skill by name to hint CrabCode should invoke it
...
- Config exists: !`test -f ${CRABCODE_PLUGIN_ROOT}/config.json && echo "✓" || echo "✗"`
- Scripts exist: !`test -d ${CRABCODE_PLUGIN_ROOT}/scripts && echo "✓" || echo "✗"`
- Tools available: !`test -x ${CRABCODE_PLUGIN_ROOT}/bin/analyze && echo "✓" || echo "✗"`
...
1. **Use ${CRABCODE_PLUGIN_ROOT} for all plugin-internal paths**
