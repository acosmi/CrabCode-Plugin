...
Practical examples of commands designed for CrabCode plugins, demonstrating plugin-specific patterns and features.
...
**Key features:**
- Uses `${CRABCODE_PLUGIN_ROOT}` for portable path
...
- Plugin's linting rules: ${CRABCODE_PLUGIN_ROOT}/config/lint-rules.json
- Security checklist: ${CRABCODE_PLUGIN_ROOT}/checklists/security.md
- Performance guidelines: ${CRABCODE_PLUGIN_ROOT}/docs/performance.md
...
Template: @${CRABCODE_PLUGIN_ROOT}/templates/review-report.md
...
- Security scan: !`bash ${CRABCODE_PLUGIN_ROOT}/scripts/security-scan.sh`
- Performance audit: !`bash ${CRABCODE_PLUGIN_ROOT}/scripts/perf-check.sh`
- Compliance check: !`bash ${CRABCODE_PLUGIN_ROOT}/scripts/compliance.sh`

**For non-production environments:**
- Basic tests: !`bash ${CRABCODE_PLUGIN_ROOT}/scripts/test-basic.sh`
- Quick lint: !`bash ${CRABCODE_PLUGIN_ROOT}/scripts/lint.sh`
...
```markdown
@${CRABCODE_PLUGIN_ROOT}/config/config-name.json
```
Use for: Loading plugin configuration files

### Pattern: Plugin Template Usage
```markdown
@${CRABCODE_PLUGIN_ROOT}/templates/template-name.md
```
...
```markdown
Check exists: !`test -f ${CRABCODE_PLUGIN_ROOT}/path/file && echo "YES" || echo "NO"`
```
...
1. **Test with plugin installed:**
   ```bash
   cd /path/to/plugin
   crabcode /command-name args
   ```

2. **Verify ${CRABCODE_PLUGIN_ROOT} expansion:**
   ```bash
   # Add debug output to command
   !`echo "Plugin root: ${CRABCODE_PLUGIN_ROOT}"`
   ```

3. **Test across different working directories:**
   ```bash
   cd /tmp && crabcode /command-name
   cd /other/project && crabcode /command-name
   ```

4. **Validate resource availability:**
   ```bash
   # Check all plugin resources exist
   !`ls -la ${CRABCODE_PLUGIN_ROOT}/scripts/`
   !`ls -la ${CRABCODE_PLUGIN_ROOT}/config/`
   ```

### Common Mistakes to Avoid

1. **Using relative paths instead of ${CRABCODE_PLUGIN_ROOT}:**
   ```markdown
   # Wrong
   !`node ./scripts/analyze.js`

   # Correct
   !`node ${CRABCODE_PLUGIN_ROOT}/scripts/analyze.js`
   ```
...
4. **Hardcoding plugin paths:**
   ...
   @/home/user/.crabcode/plugins/my-plugin/config.json

   # Correct - works everywhere
   @${CRABCODE_PLUGIN_ROOT}/config.json
   ```
