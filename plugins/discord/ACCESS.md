...
| Config file | `~/.crabcode/channels/discord/access.json` |
...
```
/discord:access set mentionPatterns '["^hey crabby\\b", "\\bassistant\\b"]'
```
...
`~/.crabcode/channels/discord/access.json`. Absent file is equivalent to `pairing` policy with empty lists, so the first DM triggers pairing.
