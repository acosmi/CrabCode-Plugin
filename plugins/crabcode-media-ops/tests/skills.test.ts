...
describe('skill routing contracts', () => {
  ...
  test('trigger evaluation set has balanced, adjacent cases', () => {
    ...
    const positives = evals.filter((item: any) => item.should_trigger)
    const negatives = evals.filter((item: any) => !item.should_trigger)
    expect(evals.length).toBeGreaterThanOrEqual(20)
    expect(positives.length).toBeGreaterThanOrEqual(negatives.length)
    expect(negatives.every((item: any) => item.near_miss)).toBe(true)
    expect(evals.some((item: any) => item.query.includes('原创'))).toBe(true)
    expect(evals.some((item: any) => item.query.includes('白底 HTML'))).toBe(true)
  })

  test('workflow instructions enforce clean-room research and delivery-before-approval', () => {
    const draftWriter = readFileSync(resolve(root, 'agents/draft-writer.md'), 'utf8')
    const research = readFileSync(resolve(root, 'media-core/skills/media-topic-research/SKILL.md'), 'utf8')
    const publish = readFileSync(resolve(root, 'platform-delivery/skills/media-publish-gate/SKILL.md'), 'utf8')
    expect(draftWriter).toContain('tools: []')
    expect(draftWriter).toContain('不得读取文件、附件、原始参考全文')
    expect(research).toContain('mediaops.research.complete')
    expect(research).toContain('零结果')
    expect(publish.indexOf('mediaops.delivery.verify')).toBeLessThan(publish.indexOf('mediaops.approval.request'))
    expect(publish).toContain('HTML')
    expect(publish).toContain('Markdown')
  })
})
