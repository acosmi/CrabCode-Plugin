...
先读取 `../../PRACTICE.md`，以 Media Gate 为不可降级约束。开始任何编排前先执行其《运行前预检》：确认工具可发现、调用 `mediaops.capabilities`、核对身份与依赖；预检失败按停止码停止，仅可经用户确认输出未治理草稿，手工替代结果只标 diagnostic。

## 路由

1. 参考先用 `mediaops.reference.register` 分类并建立不可复制清单；研究与选题：`media-topic-research`。
2. 本地文件稿（Markdown）进入治理：读取文件正文后用 `mediaops.content.save` 登记为 intake（local-editorial 单人模式传 `serviceImport: true` 交由服务执行体做机械导入，使真人可继续担任研究完成人），再走 `mediaops.reference.register` → `mediaops.research.capture`/`mediaops.research.complete` → 保存 drafted → `mediaops.originality.scan`(contentId)。扫描只接受 contentId，不接受文件路径；任何改稿生成新 revision 并使旧扫描 stale，不得绕过状态机。
...
默认向用户呈现已验证的精排白底 HTML，Markdown 仅作备份。交付必须包含 contentId、revisionId、contentHash、articleDocHash、renderManifestHash、profileVersion、自动 QA 证据摘要、身份 assurance、当前阶段、未解决风险和下一步。遇到停止码立即暂停，不靠文字承诺绕过工具门禁；报告中每个已完成阶段必须附服务端权威 ID/哈希，缺凭据的阶段一律记 `GATE_NOT_EXECUTED`，绝不写成已完成。
