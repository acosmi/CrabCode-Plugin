...
先读取 `../../../media-core/PRACTICE.md`、`../../PRACTICE.md` 与 `../../references/creator-style-form.md`。开始前先按《运行前预检》确认 `mediaops.capabilities` 可用与身份就绪；预检失败按停止码停止，不得离线伪造表单提交状态。

1. 选择模式：快速建档、完整建档、增量更新。
2. 调用 `mediaops.style.form.template` 生成非技术用户可打开的本地表单预览。
3. 逐步收集定位、创作者类型、读者、表达维度、结构、语言禁区、真实性边界、平台差异与存储同意；历史样本先登记 referenceId、角色、权利和允许用途，表单只保存这些元数据。
4. 未完成时调用 `save_draft`；恢复时先 `get`，不得丢失已填字段。
5. 由具备 `profile_editor` 角色的可信 principal 完成草稿与 `submit`。提交状态仍是待确认，不是正式 profile；调用参数中的姓名不构成身份。
