...
先读取 `../../../media-core/PRACTICE.md`、`../../PRACTICE.md` 与 `../../references/publish-runbook.md`。门禁开始前先按《运行前预检》执行完整 preflight（工具可发现 → `mediaops.capabilities` → 身份 → `mediaops.doctor` 依赖就绪）；预检失败按停止码停止，未由工具真实执行的门禁一律记 `GATE_NOT_EXECUTED`，报告中的已完成阶段必须逐项附权威 ID/哈希。
