...
This plugin was migrated from the upstream Claude-branded `typescript-lsp` source at `bangong/claude-plugins-official/plugins/typescript-lsp`. The upstream README and LICENSE files are not redistributed in this product; only the plugin name and metadata describing the wrapped server are retained.
