...
Users running `onboard.py` directly in their own terminal (not via CrabCode) will see all output live — no changes needed there.
...
The known apps name `buddy` resolves to the `buddy/device/` directory in this repo (custom launcher + Hello + CrabCode Buddy BLE client + Snake). Any other `--apps` value is treated as a filesystem path.
...
- **CrabCode Buddy over BLE.** First time only: in CrabCode Desktop, **Help → Troubleshooting → Enable Developer Tools** (one-time, persists across launches). Then **Developer menu → Hardware Buddy → Connect**. BLE works regardless of the WiFi state — the link to CrabCode.app is local.
...
Python itself has to exist before this skill can do anything — you can't bootstrap an interpreter from inside one. `git` is **not** required — the `/maker-setup` command falls back to downloading the GitHub tarball with `curl`+`tar` (both pre-installed on macOS, Linux, and Windows 10+) when `git --version` fails. CrabCode's responsible for detecting Python and installing it if missing *before* running any `scripts/*.py` invocation. Detection is just running `python3 --version` / `python --version` — if it fails, CrabCode fetches Python with the host's native package manager before anything else.

**Per-OS Python bootstrap (CrabCode's responsibility if missing):**
...
- **Bundle path resolution.** `install_apps.py`'s `--src buddy` shorthand resolves in this order:
  ...
  2. The `buddy/device/` directory inside this repo, found via `os.path.realpath(__file__)` walking up from `install_apps.py`. Works for any clone location, including symlinked skill installs at `~/.crabcode/skills/m5-onboard/`.
