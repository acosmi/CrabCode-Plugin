...
display-name: Cardputer 应用迭代
description: Iterate on the Cardputer-Adv MicroPython app bundle (CrabCode Buddy, Snake, Hello) after the device is already provisioned via m5-onboard. Use when the user wants to add a new app, push a single changed .py without re-flashing, watch device serial logs, or run a one-shot REPL command. Trigger on "add an app", "push to the cardputer", "tail the device", "run on the device", or follow-up work after /maker-setup.
---

# Cardputer Buddy app bundle

The `buddy/` directory in the local `build-with-crabcode` clone is the MicroPython payload that `m5-onboard` installs onto `/flash/`. Work inside that clone.
...
└── apps/
    ├── crabcode_buddy.py  BLE client → CrabCode Desktop's Hardware Buddy
    ├── hello_cardputer.py
    └── snake.py
```
