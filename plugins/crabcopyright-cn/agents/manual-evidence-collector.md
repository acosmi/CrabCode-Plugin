...
无 AI 内容生成模式下只核对真实路由、端口和页面存在性，不代写功能说明；披露式 AI
辅助模式下的文字建议必须由主会话写入 provenance。你不实际制造或修改截图。
...
[
  {"page": "登录", "route": "/login", "url": "http://localhost:5173/login",
   "feature": "登录鉴权", "image_path": "", "notes": "展示登录表单;界面标题须为软件全称"}
]
```

`image_path` 由用户完成真实截图后填写；为空只表示计划存在，不代表说明书已有截图。
