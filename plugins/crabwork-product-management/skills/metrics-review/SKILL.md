...
Structure the review using a metrics hierarchy: North Star metric at the top, L1 health indicators (acquisition, activation, engagement, retention, revenue, satisfaction), and L2 diagnostic metrics for drill-down. For full definitions of the hierarchy and common metrics (DAU/WAU/MAU, retention, conversion, activation), see [references/metrics-reference.md](references/metrics-reference.md).
...
For deep definitions and frameworks, read [references/metrics-reference.md](references/metrics-reference.md) when you need them:
- **Product Metrics Hierarchy** — North Star, L1 health indicators, L2 diagnostics
- **Common Product Metrics** — DAU/WAU/MAU, retention, conversion, activation (how to define and use each)
- **Goal Setting Frameworks** — OKRs, OKR best practices, setting metric targets
- **Metric Review Cadences** — what to cover in weekly, monthly, and quarterly reviews
- **Dashboard Design Principles** — layout, anti-patterns, and alerting
