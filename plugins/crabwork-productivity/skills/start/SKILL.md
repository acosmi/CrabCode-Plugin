...
**If `dashboard.html` doesn't exist:** Copy it from `${CRABCODE_PLUGIN_ROOT}/skills/dashboard.html` to the current working directory.

**If `CRABCODE.md` and `memory/` don't exist:** This is a fresh setup — after opening the dashboard, begin the memory bootstrap workflow (see below). Place these in the current working directory.

### 3. Open the Dashboard

Do NOT use `open` or `xdg-open` — the agent may run in a sandboxed or remote environment where shell open commands won't reach the user's browser. Instead, tell the user: "Dashboard is ready at `dashboard.html`. Open it from your file browser to get started."
...
- /crabwork-productivity:update to sync tasks and check memory
- /crabwork-productivity:update --comprehensive for a deep scan of all activity
```
...
Only do this if `CRABCODE.md` and `memory/` don't exist yet.
...
**CRABCODE.md** (working memory, ~50-80 lines):
```markdown
# Memory
...
Use /crabwork-productivity:update to keep things current (add --comprehensive for a deep scan).
```
