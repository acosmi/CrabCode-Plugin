...
```bash
/crabwork-productivity:update
/crabwork-productivity:update --comprehensive
```
...
Read `TASKS.md` and `memory/` directory. If they don't exist, suggest `/crabwork-productivity:start` first.
...
- **Status changes** ("launch done") → update project status, demote from CRABCODE.md
