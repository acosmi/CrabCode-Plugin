...
- 私有 Core 候选分支：`codex/crabcode-security-runtime-20260724`
- 私有 Core 实现提交：`869045f3e857a3c81d8d8e83d78cbeed8d64219f`（已推送）
- 推送后边界：两条分支均未 review、未合并、无 commit-bound 远程 CI 或签名制品、未发布；Core `WORKFLOW_SCRIPTS` 仍默认关闭，Marketplace 仍为 `staged-not-active`
...
| Core 候选提交已补 worker/vm、强制取消和权限不升级，但缺少 review、commit-bound CI、制品与 hostile-code 边界 | 容易把进程内隔离写成 sandbox，补丁测试仍可接触宿主 | 已推送提交只固化实现快照；可信仓库 GA 仍需 review、commit-bound CI、正式版本门槛、发布制品和 CLI/GUI E2E。不可信仓库支持另需 OS 隔离与 clean profile，两者均未交付 |
...
其中 `normalize()` 只能执行事先声明的 `baseline/`、`target/` 路径根转换和排除 `docs/legal/**` 自指来源材料，不得删除、重排或改写产品正文；文件路径、内容、mode 和 symlink 类型都参加比较。校验器在空临时目录先按锁定对象 materialize 固定上游 25 个 blob，再应用 `FULL-PORT.patch`，最后与 25 个映射目标逐字节比较。generator 的 `--check` 还必须从同一输入复算完全相同的 patch。`PORTING-MAP.md` 用于人审，重放等式用于机审，二者缺一不可。所有产品改动收敛后才生成最终 patch 和哈希；当前 seal 已进入公开候选分支的实现提交 `49b9c68afbbf648b34033686007334545d5e0842` 并推送，内容已 commit-pinned，但因未 review、无 commit-bound CI、未合并且未发布，仍不能称不可变发布封存。
...
本次实施已固化在私有 Core 候选分支 `codex/crabcode-security-runtime-20260724` 的提交 `869045f3e857a3c81d8d8e83d78cbeed8d64219f` 并推送；该候选提交保留测试的能力包括：
...
- plugin Agent `initialPrompt` 和任务/进度桥接已进入候选提交实现；
...
上述内容已由不可变候选提交固定；其测试结果来自提交前相同字节范围的本地验证，不是 commit-bound 远程 CI，也不是可供 Marketplace 依赖的正式版本或制品。仍未关闭：

- 对 Core 候选提交的独立 review、commit-bound 远程 CI、合并/正式版本和制品 SHA256；
...
- 发布制品：Core 候选提交、review/CI、正式版本、制品 SHA256 和真实 G1/E2E 记录。

当前权威实现落点是私有仓 `/Users/fushihua/Desktop/CrabCode` 的候选分支 `codex/crabcode-security-runtime-20260724` 与提交 `869045f3e857a3c81d8d8e83d78cbeed8d64219f`；该提交精确包含 50 条实施路径。提交前的 49 个现存文件加一个 `.ts`→`.tsx` 删除侧，以及验证暂存副本 `/private/tmp/CrabCode-security-runtime-copy`，其实施范围聚合 SHA256 均为 `e0a8dd433b2d7adc32935d6aed9a97cc86fb9ee6949c7fcef553f129b051da42`；这个值是历史预提交字节快照，只用于交叉复核，不是 Git commit ID。实际仓内用户既有的 `package.json`、`bun.lock` 和文档变更未进入该提交。提交虽已推送，但仍未 review、无 commit-bound 远程 CI、未合并、无发布制品且功能默认关闭。
...
当前状态：Core 候选提交 `869045f3e857a3c81d8d8e83d78cbeed8d64219f` 已推送，确定性 host/Core 本地测试可用于开发复核；但尚无绑定该提交的 review/远程 CI、正式版本 Gate、发布制品和真实制品 E2E，因此 G1/G2 的发布口径仍不通过。
...
- [x] 在私有 Core 候选提交 `869045f3e857a3c81d8d8e83d78cbeed8d64219f` 实现 Workflow 自动发现
- [x] 在私有 Core 候选提交 `869045f3e857a3c81d8d8e83d78cbeed8d64219f` 实现 args、log、phase
- [x] 在私有 Core 候选提交 `869045f3e857a3c81d8d8e83d78cbeed8d64219f` 实现 agent、pipeline、parallel
- [x] 在私有 Core 候选提交 `869045f3e857a3c81d8d8e83d78cbeed8d64219f` 实现 structured output schema
- [x] 在私有 Core 候选提交 `869045f3e857a3c81d8d8e83d78cbeed8d64219f` 实现 worker/vm、deadline、forced cancel
- [x] 在私有 Core 候选提交 `869045f3e857a3c81d8d8e83d78cbeed8d64219f` 收紧宿主桥接/JSON 边界并阻断构造器逃逸
- [x] 在私有 Core 候选提交 `869045f3e857a3c81d8d8e83d78cbeed8d64219f` 实现未等待 RPC 的收敛、失败传播、取消与限时 drain
- [x] 在私有 Core 候选提交 `869045f3e857a3c81d8d8e83d78cbeed8d64219f` 实现嵌套 Agent 调度槽借用、防单槽死锁，并关闭显式/隐式 background、remote isolation 和 handoff 旁路
- [x] 在私有 Core 候选提交 `869045f3e857a3c81d8d8e83d78cbeed8d64219f` 实现 caller→child 权限不升级和嵌套 Agent allowlist
- [x] 在私有 Core 候选提交 `869045f3e857a3c81d8d8e83d78cbeed8d64219f` 实现调用/大小/时长上限、预算单飞与每轮 guard，并拒绝模型策略覆盖
- [x] 在私有 Core 候选提交 `869045f3e857a3c81d8d8e83d78cbeed8d64219f` 区分真实取消与清理性 abort，保留普通失败原错和 failed 状态
- [x] 在私有 Core 候选提交 `869045f3e857a3c81d8d8e83d78cbeed8d64219f` 实现 Task 状态、进度和 `initialPrompt`
- [x] 在私有 Core 候选提交 `869045f3e857a3c81d8d8e83d78cbeed8d64219f` 接通直接 QueryEngine/无头/SDK 的主 Agent 工具作用域
- [x] 在私有 Core 候选提交 `869045f3e857a3c81d8d8e83d78cbeed8d64219f` 接通 AppServer 已安装/cwd Agent 的逐回合身份解析、作用域工具和 ALS 隔离
- [x] 在插件侧映射 Hook 事件
- [x] 将 Core 实现固化为候选提交并推送独立分支，未合并
- [ ] 完成 Core 候选提交的独立 review、commit-bound CI、合并/正式版本和发布制品
...
- [x] Core 候选提交 `869045f3e857a3c81d8d8e83d78cbeed8d64219f` 的相同预提交字节快照已运行 CPU loop、永不 resolve Promise 的硬取消和权限不升级测试；该结果不是远程 CI 或制品测试
...
本层只回答“固定上游 25 个 blob 是否完整进入目标派生”。当前 seal 已通过 505 项检查，并固化在已推送的公开候选实现提交 `49b9c68afbbf648b34033686007334545d5e0842`；由于该提交尚未 review、无 commit-bound CI、未合并且未发布，它仍不是最终发布封存。任何 25 个产品文件或三项来源工具变化都必须重新生成/校验 seal，否则本结论自动失效。
...
3. Core 候选提交 `869045f3e857a3c81d8d8e83d78cbeed8d64219f` 的相同预提交字节快照中，worker/vm、硬取消、权限非升级、嵌套 allowlist、调度、预算和 AppServer 已安装 Agent 路径通过确定性本地测试；
...
当前两条候选提交只关闭了“实现快照已进入 Git 并推送”的子条件；上述最终 review、commit-bound CI、合并/正式版本、制品、E2E、质量和运营签署条件均未因此自动关闭。
...
本次复核按“来源能否重建 → 宿主能否执行 → 失败是否安全 → 输出是否有所有权 → 关联方能否运营 → 发布能否回滚”逆向取证。勾选只表示对应源码或证据已在审计时点出现；候选提交和提交前本地测试不等同于独立 review、commit-bound CI、合并、发布制品或 GA。
...
| 公开插件候选提交 | `codex/crabcode-security-full-port-20260724` / `49b9c68afbbf648b34033686007334545d5e0842` | 已推送的实现快照；不是 review、CI、合并或发布证明 |
...
| Core 候选提交 | `codex/crabcode-security-runtime-20260724` / `869045f3e857a3c81d8d8e83d78cbeed8d64219f`；代表路径包括 `/Users/fushihua/Desktop/CrabCode/src/tools/WorkflowTool/runtime.ts`、`tests/unit/workflow-runtime.test.ts`、`tests/unit/workflow-budget-query.test.ts`、`tests/unit/workflow-agent-contract.test.ts`、`tests/unit/StreamingToolExecutor-reentrantAgentSlot.test.ts`、`tests/unit/query-engine-main-agent-tools.test.ts`、`apps/agent-worker/tests/headless-runtime.test.ts` | 已推送的 50 路径实现快照；提交前相同字节范围的本地测试覆盖 worker/vm、宿主桥接收紧、硬取消、RPC drain、预算 guard、嵌套全生命周期调度和 direct/AppServer Agent 合同；不是远程 CI、合并或发布证据 |
...
- 私有 Core：候选提交前的相同实施字节快照 `bun run test` 为 `5616 pass / 3 skip / 13 todo / 0 fail`，共 5632 tests、508 files、19087 expects；`bun run test:worker` 在允许 loopback/watch 能力的环境为 `1695 pass / 2 skip / 0 fail`，14 项 audit 测试、Core/worker typecheck、两项通信审计和 diff-check 均通过；与实际实施范围逐字节一致的验证暂存副本中，定向主/worker 分别 `187/187`、`132/132`，47 个相关 TS/TSX 文件的 Biome 检查通过。该字节范围随后固化为 `869045f3e857a3c81d8d8e83d78cbeed8d64219f`；这些结果仍是预提交本地验证，不冒充远程 CI 或制品测试。暂存副本曾在受限沙箱内因 loopback listen `EPERM` 与 watcher 环境出现 5 个环境失败，已保留该发现但不把环境拒绝写成产品失败；
...
| 来源锁与重放 seal | `CLOSED — commit-pinned reproducibility evidence` | 固定上游完整 checkout 经 verifier 通过 `505` 项检查；25 个目标文件、mode、staged Marketplace 规范化条目、promotion selector、三项来源工具哈希和 `FULL-PORT.patch` 均已入锁；在空临时目录 materialize 上游 blob 后重放逐字节相等，并已固化在公开候选实现提交 `49b9c68afbbf648b34033686007334545d5e0842`。review、commit-bound CI、合并和发布封存仍开放 |
| Core 取消、边界与调度 | `IMPLEMENTED — preview / ACCEPTANCE OPEN` | Core 候选提交 `869045f3e857a3c81d8d8e83d78cbeed8d64219f` 已固化 worker/vm、deadline/forced termination、宿主桥接收紧、RPC drain、普通失败保真、预算租约/每轮 guard、嵌套单槽防死锁及所有 detached 旁路拒绝、caller→child 权限不升级和嵌套 Agent allowlist；预算模式会把受预算顶层 Agent 调用串行化，10 秒 drain 与进程级保守成本账本仍是明确边界 |
...
“IMPLEMENTED — preview”描述默认关闭、未完成验收的成熟度，不再描述 Git 提交状态；实现已由候选提交固定，但提交前本地测试不等于独立 review、commit-bound CI、合并、制品或真实 E2E 已验收。

### 14.3 仍开放的发布阻断项

| 阻断项 | 状态 | 关闭条件 |
|---|---|---|
| Core 候选提交推送 | `CLOSED — candidate snapshot` | 50 条实现路径已固化为 `869045f3e857a3c81d8d8e83d78cbeed8d64219f` 并推送独立分支，未合并 |
| Core review/CI/制品/发布 | `OPEN` | 对候选提交完成独立 review、commit-bound 远程 CI、合并/正式版本、构建并保存制品 SHA256 |
...
| 结论 | 当前值 | 含义 |
|---|---|---|
| `source-set/replay-complete` | **YES** | 固定上游 25/25 blob、目录外条目、一一映射、mode、目标字节和完整适配 patch 均可从锁定对象重建，并已进入公开候选实现提交 `49b9c68afbbf648b34033686007334545d5e0842`；review、CI、合并与发布封存仍开放 |
| `deterministic-host-contract-tested` | **YES** | 固定脚本、Workflow host simulator、边界与 Core 候选提交 `869045f3e857a3c81d8d8e83d78cbeed8d64219f` 相同预提交字节快照的确定性合同已经执行本地测试；不外推为远程 CI、随机模型质量或真实制品等价 |
| `model-behavior-equivalence` | **PENDING** | 尚未完成以锁定上游为 baseline 的版本化盲测配对 corpus、功效分析和独立解盲签署 |
| `GA-not-complete` | **YES** | Core 独立 review、commit-bound CI、合并/正式版本与制品、版本 Gate、AppServer 自定义 `--agents` 平台合同、真实制品 E2E、质量盲测、owner 签署、canary 和回滚尚未全部完成 |
