...
- 结论:22 文件 git numstat 每个 1/1(仅 description 行);全 crablaw-cn **105 个 SKILL.md description 100% 含 when 从句**;validate-all 通过、bun test 33 pass/0 fail。已提交 `bee6950`。

---

## 终点毁灭性复核审计(主控自审,敌意视角)

### 真实验证输出(git diff main...HEAD + grep + 门禁)
- 规模:124 files changed, 4410 insertions(+), 23 deletions(-);5 提交(批次0-3 + 收尾)。
- 品牌词(claude/anthropic/sonnet/opus/haiku/codex)全 crablaw-cn **零命中**;境外 SaaS(Westlaw/CourtListener/DocuSign/Ironclad/iManage/Slack)**零命中**。
- 红线分类:法律红线(全角)86 文件 = 105 − 11(study 教育免责头)− 8(builder-hub 无红线),三者精确吻合预期;builder-hub 无法律红线误含。
- 规模:13 个 plugin.json(matter-core + 12 子板块)、105 个 SKILL.md。
- 门禁:validate-all all checks passed;bun test 33 pass/0 fail。

### 一、根因证伪
- **板块缺失→中国法重写(非复制)**:反证——若只复制 _legacy 美国法,会带入英美实体规则/境外连接器/旧品牌;全 crablaw-cn 品牌+境外 SaaS 零命中、105 skill 全中国法实体规则,证明确为重写。同类变体:9 板块全覆盖。
- **description undertrigger→补 when+pushy**:反证——若不补,中文 query 不触发;105/105 含 when 从句。
- **review-queue/matterType enum 同类变体**:反证——若不补全,批次2/3 落板块时跨板块复核队列字段不一致复发;enum 一次补全 + 14 个 allOf 分支,复发路径关闭。
- **半角红线(agent 谎报)→主控字符级校验**:反证——若不字符级核,红线不统一;全角红线 86 文件一致、无半角残留。

### 二、关联方与回归
- 触碰底座:matter.schema(matterType enum 加值,纯增量不破坏旧实例)、review-queue.schema(enum 加值 + 新 allOf 分支,不影响旧 4 板块)、brandGuard.ts(加 docs 中文规划文档 ignore,**不放松 plugins/ 检查**,带回归测试)。
- 回归:validate-all + bun test 全绿;brandGuard 改动有专门回归断言。
- 全量 diff 重读:124 文件,23 删除主要为 22 个 description 旧行替换 + schema enum 行调整,无误伤/调试残留。

### 三、需求对齐(逐条)
- 追加需求「剩余 9 板块全部中国法化」:**满足**——corporate/litigation/ip/regulatory/ai-governance/product 中国法化;legal-aid/legal-study 语义重建;builder-hub 去美国法化。
- 工作框架(根因审计/细化/偏离判断/subagent 派发+主控重核):**满足**——全程执行,本日志记录。
- skill-creator 深度优化:**满足**——105 description 全优化。
- 范围:无静默扩缩;Matter Gate 豁免(study/builder-hub)、已上线 description 优化均已论证。

### 四、过度工程反向审
- schema 只造 4 个有 ≥2 板块复用论证者,板块专属对象留板块内;CRABCODE/README 评估后不改(不维护板块清单);review-queue allOf 对齐既有范式。无为"显得完整"而加的抽象/配置/依赖。

### 五、交付完整性(git)
- 核心产出全在 HEAD `bee6950`;工作区仅 2 个 docs/audit 陌生文件(他人窗口产物,全程未触碰);无 stash/worktree 残留。

### 结论:**通过**(附 3 项已知限制,诚实标注,非本任务可闭环)
1. **法条口径为抽样核验 + 全产物【需律师复核】红线兜底**,非 105 个逐字法律终审——这是 AI 法律产品定位(辅助草稿,执业律师终审),非缺陷;抽样的 8 个高风险 skill 口径均专业准确。
2. **PRACTICE.md 的 `lint-tool-scope.ts` 失实自述未修**(批次0 已记):Matter Gate 无 CI 强制,靠 SKILL.md 约定 + 主控 grep 验收。是否补强制脚本属新增基础设施,留用户决定。
3. **schema 无 CI 消费者**(底座既有性质):新增 schema 是数据契约设计资产,靠 SKILL.md 引用,无机器校验。

---

## 合并安全核验与建议(交回用户决定——破坏性操作不自动执行)

- 任务分支:`task/crablaw-cn-localize-all-20260621`,基于 `main`(c693ea3)。
- 核验:`main` 是 `HEAD` 祖先 → **可 --ff-only 合并,无分叉**;`HEAD..main` = 0(无他人改动会丢失);未触碰 `docs/audit/*`(他人窗口产物)。
- `origin/main` 落后本地 `main` 1 提交(c693ea3,上会话遗留未推送)。
- **建议合并步骤(用户确认后执行)**:
  ```
  git checkout main
  git merge --ff-only task/crablaw-cn-localize-all-20260621
  # 可选推送(注意会一并推送上会话遗留的 c693ea3):
  # git push origin main
  ```
- 两个 `docs/audit/*` 未跟踪文件非本任务产物,主控不处置,留其所属窗口/用户决定。

---

## 补遗:限制#2 闭环(用户指令「完善不要留尾巴」,2026-06-21)

**Change**:把 `matter-core/PRACTICE.md` 声明却从未实现的设计契约 `scripts/lint-tool-scope.ts` 真正落地为门禁,并把 PRACTICE.md 自述改为如实且完整。

**Root cause**:原文第 7 行断言"the only always-on enforcement point is...(by `scripts/lint-tool-scope.ts`)",但该脚本全仓不存在——既是失实自述,也是实质门禁缺口(将来新增实体技能漏掉 `## Matter Gate` 即静默绕过共享 guardrails)。仅软化措辞为"靠约定"=创可贴:缺口仍在,会在"下次有人新增技能"处复发。根因修复=实现该脚本使断言为真且关掉缺口。

**落地**(照搬既有校验器范式):
- `src/policy/matterGateValidator.ts`:扫描 `plugins/crablaw-cn/**/SKILL.md`,非豁免技能须含 `## Matter Gate` 且引用 `matter-core/PRACTICE.md`(空标题=hollow gate 也拦)。
- `scripts/lint-tool-scope.ts`:瘦 CLI(文档点名的精确路径,使自述逐字成真)。
- 接入 `scripts/validate-all.ts`(`[tool-scope]` 段)+ `package.json` `lint:tool-scope`。
- `tests/validators/matter-gate.test.ts`:8 例(合规/缺标题/缺引用/两类豁免/intake/matter-core 实体技能仍强制/family 缺席)。

**豁免模型(比原文"non-intake"更精确)**:gate 豁免板块 `builder-hub`、`cn-legal-study`(非法律服务);intake 技能 matter-core 的 new-client/new-matter/conflict-check/matter-archive/review-queue + cn-contract/cold-start-interview。其余 80 个(含 matter-core 的 compliance-deadline-watcher / matter-deep-analysis)强制。当前活树 80/80 通过=绿色基线。PRACTICE.md 第 7 行同步改写,补上板块豁免说明(原文遗漏)。

**独立重核(第四阶段,非采信自检)**:① `bun run scripts/lint-tool-scope.ts .` exit=0;② 活树对抗——临时删真实技能 entity-compliance 的 gate 标题→报"missing"(exit=1)、删引用→报"must reference"(exit=1)、`git checkout` 还原→exit=0、victim 干净;③ `bun test ./tests/validators/` 31 pass/0 fail(含新 8 例)。

**为何不复发**:新增任何实体技能若漏 gate 或 gate 不引用 PRACTICE.md,`bun run validate` 即 exit=1 拦下;反证——若仍靠人工 grep,则"下次换人/换窗口新增技能"处复发。现已机器强制。

**已知限制#2 状态:已闭环**(限制#1 法条抽样+红线兜底=产品定位,非缺陷;限制#3 schema 无消费者属另一资产范畴,本次未扩,不在用户指令内)。

**门禁现状说明**:`bun run validate` 当前仍报 1 处 `[brand]`——`plugins/crabwork-engineering/README.md` 的 anthropics 归因行。该目录为**未跟踪**(`??`),系另一窗口在途 knowledge-work 转化产物,先于本次改动即存在(stash 本次改动后仍红)。按「绝不覆盖其它窗口改动」原则,主控不触碰,留其所属窗口处置。本次 matter-gate 改动与之完全隔离。
