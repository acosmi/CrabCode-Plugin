...
These marketplace entries point to external Git repositories, organization-relative paths, provider package paths, or source descriptors that are not present in the current `bangong/legacy-plugins-official` clone.
...
| EX-003 | `aikido` | `https://github.com/AikidoSec/aikido-legacy-plugin.git` | none |
...
| EX-009 | `atomic-agents` | `legacy-plugin/atomic-agents` | development |
| EX-010 | `azure` | `https://github.com/microsoft/azure-skills.git` | deployment |
| EX-011 | `azure-cosmos-db-assistant` | `https://github.com/AzureCosmosDB/cosmosdb-legacy-assistant-plugin.git` | database |
...
| EX-017 | `circleback` | `https://github.com/circlebackai/legacy-assistant-plugin.git` | productivity |
| EX-018 | `clickhouse` | `https://github.com/ClickHouse/clickhouse-legacy-assistant-plugin.git` | database |
...
| EX-022 | `cockroachdb` | `https://github.com/cockroachdb/legacy-plugin.git` | database |
...
| EX-030 | `datadog` | `https://github.com/datadog-labs/legacy-assistant-plugin.git` | monitoring |
...
| EX-037 | `firecrawl` | `https://github.com/firecrawl/firecrawl-legacy-plugin.git` | development |
...
| EX-040 | `intercom` | `https://github.com/intercom/legacy-plugin-external.git` | productivity |
| EX-041 | `jfrog` | `jfrog/legacy-plugin` | security |
| EX-042 | `microsoft-docs` | `https://github.com/MicrosoftDocs/mcp.git` | development |
| EX-043 | `mintlify` | `https://github.com/mintlify/mintlify-legacy-plugin.git` | development |
| EX-044 | `miro` | `legacy-plugins/miro` | design |
...
| EX-050 | `notion` | `https://github.com/makenotion/legacy-assistant-notion-plugin.git` | productivity |
| EX-051 | `oracle-ai-data-platform-workbench-spark-connectors` | `ai/legacy-assistant-plugins/oracle-ai-data-platform-workbench-spark-connectors` | development |
| EX-052 | `outputai` | `coding_assistants/legacy-assistant/plugins/outputai` | development |
| EX-053 | `pagerduty` | `https://github.com/PagerDuty/legacy-assistant-plugins.git` | monitoring |
| EX-054 | `pigment` | `https://github.com/gopigment/ai-plugins.git` | productivity |
| EX-055 | `pinecone` | `https://github.com/pinecone-io/pinecone-legacy-assistant-plugin.git` | database |
| EX-056 | `planetscale` | `https://github.com/planetscale/legacy-plugin.git` | database |
...
| EX-059 | `postman` | `https://github.com/Postman-Devrel/postman-legacy-assistant-plugin.git` | development |
| EX-060 | `prisma` | `https://github.com/prisma/legacy-plugin.git` | none |
...
| EX-066 | `remember` | `https://github.com/Digital-Process-Tools/legacy-assistant-remember.git` | none |
...
| EX-074 | `sentry` | `https://github.com/getsentry/sentry-for-legacy-assistant.git` | monitoring |
| EX-075 | `servicenow-sdk` | `providers/legacy-assistant/plugin` | development |
...
| EX-080 | `sonatype-guide` | `https://github.com/sonatype/sonatype-guide-legacy-plugin.git` | security |
| EX-081 | `sourcegraph` | `https://github.com/sourcegraph-community/sourcegraph-legacyassistant-plugin.git` | development |
| EX-082 | `spotify-ads-api` | `https://github.com/spotify/ads-legacy-plugin.git` | productivity |
| EX-083 | `stripe` | `providers/legacy-assistant/plugin` | development |
| EX-084 | `sumup` | `providers/legacy-assistant/plugin` | development |
...
| EX-090 | `windsor-ai` | `https://github.com/windsor-ai/legacy-assistant-windsor-ai-plugin.git` | productivity |
| EX-091 | `wix` | `https://github.com/wix/skills.git` | development |
| EX-092 | `wordpress.com` | `https://github.com/Automattic/legacy-assistant-wordpress.com.git` | none |
...
- Remove legacy assistant product identity from user-facing text.
...
The "Source" column in §"Worker Assignments" is a single-string render. The authoritative `bangong/legacy-plugins-official/.legacy-plugin/marketplace.json` stores each entry's `source` as a typed object that pins a commit `sha`, an optional `ref`, and an optional sub-path:
...
Implication: where the single-string render shows a path like `legacy-plugin/atomic-agents`, `revenuecat`, or `providers/legacy-assistant/plugin`, that string is the upstream sub-path, not the upstream URL. The full structured record (URL + path + sha) is the only safe input to the conversion ticket.
...
4. Classification scans the path-pinned subtree when `path` is set, else the repo root. It skips non-legacy assistant-IDE plugin manifest dirs (`.cursor-plugin`, `.legacy-assistant-plugin`, `.zed-plugin`, `.copilot-plugin`, `.continue-plugin`, `.windsurf-plugin`, `.cline-plugin`, `.aider-plugin`) so that mirrored IDE manifests do not inflate `plugin.json` counts.
